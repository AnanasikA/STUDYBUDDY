import Head from 'next/head';
import Link from 'next/link';

export default function LangPL() {
  return (
    <>
      <Head>
        <title>StudyBuddy – Język polski</title>
      </Head>

      <main className="max-w-2xl mx-auto px-4 py-16 text-center">
        <h1 className="text-3xl md:text-4xl font-extrabold mb-6 text-gray-900" style={{ fontFamily: '"Playfair Display", serif' }}>
          Wersja polska
        </h1>
        <p className="text-gray-600 mb-6">
          Korzystasz z wersji polskiej serwisu StudyBuddy. Możesz zmienić język w menu u góry strony.
        </p>
        <div className="flex justify-center gap-4">
          <Link href="/lang/en" className="bg-gray-200 hover:bg-gray-300 px-4 py-2 rounded-md text-sm">English</Link>
          <Link href="/lang/ua" className="bg-gray-200 hover:bg-gray-300 px-4 py-2 rounded-md text-sm">Українська</Link>
        </div>
      </main>
    </>
  );
}
